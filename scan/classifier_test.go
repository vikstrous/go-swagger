...
func TestClassifierInclude(t *testing.T) {
	...
	classifier := &programClassifier{
		Includes: packageFilters([]packageFilter{
			packageFilter{"github.com/vikstrous/go-swagger/fixtures/goparsing/classification"},
			packageFilter{"github.com/vikstrous/go-swagger/fixtures/goparsing/classification/transitive/mods"},
			packageFilter{"github.com/vikstrous/go-swagger/fixtures/goparsing/classification/operations"},
		}),
	}
	classified, err := classifier.Classify(prog)
	assert.NoError(t, err)
